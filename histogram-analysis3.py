...
def getcloudprop (r063,r390,temp,mylat,mylon,site,name,outputfilehandle):
    r063,r390,temp=np.squeeze(r063),np.squeeze(r390),np.squeeze(temp)
    lat, lon = getlatlon(name)
    rlen=np.sqrt(lat*lat/(90*90) + lon*lon/(180*180))
    rang=np.arctan(lat/lon)
    ...
    for b in boxsize:
        ...
        sortedice   = np.sort(tempb[ice]  ,axis=None)
        sortedwater = np.sort(tempb[water],axis=None)
        
        iceset   = sortedice[int(0.9*len(sortedice))-1:len(sortedice)-1]
        waterset = sortedwater[0:int(0.1*len(sortedwater))+1]
        ...
        outstring = sat+','+site+','+season+','+year+','+julian+','+hhmm+','+str(b)+','
        outstring = outstring + coldwatern+','+coldwater+','+coldwatersd+','+hoticen+','+hotice+','+hoticesd+'\n'
        
        outputfilehandle.writelines(outstring)
    return
...
icedry   = 20.0  # VIS/IR above this limit indicates ice pixel
icewet   = 20.0  # VIS/IR above this limit indicates ice píxel
waterdry =  5.0  # VIS/IR below this limit indicates water pixel
waterwet =  7.0  # VIS/IR below this limit indicates water pixel

#Define the output filename and header
outfilename = 'Temperature-phase.csv'
if os.path.isfile(outfilename):
    outfn = open(outfilename, 'a')
else:
    header='sat,site,season,year,julian,hhmmss,boxsize,N_coldwater,Avg_coldwater_Temp(K),Std_coldwater(K),N_hotice,Avg_hotice_Temp(K),Std_hotice(K)\n'
...
while count < len(listch1):
    ...
    getcloudprop(r063,r390,temp,mylat,mylon,site,listch2[count],outfn)
