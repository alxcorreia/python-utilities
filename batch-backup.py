...
def goesimg ( file, type ):
    ...
    if procmode == 'VIS':
        if type == 'RAW':
            cs = m.pcolormesh(xi,yi,np.squeeze(Raw),cmap='gray',vmin=-50, vmax=1100)
            myunits=RawUnits
        ...
        if type == 'RAD':
            RadPre=mVIS*(Raw-X0)
            RadVIS=RadPre * C
            cs = m.pcolormesh(xi,yi,np.squeeze(RadVIS),cmap='gray', vmin =-50, vmax=750)
            myunits=RadVISUnits
            myticks=[0,100, 200,300,400, 500, 600, 700]
            mylabels=["0","100","200","300","400","500","600","700"]
        if type == 'ALB':
            ...
            cs = m.pcolormesh(xi,yi,np.squeeze(RefVIS),cmap='gray',vmin=-0.1,vmax=1.1)
            myunits=RefVISUnits
    ...
    if procmode == 'IR':
        if band == 'BAND_02':
            ...
            if type == 'RAD':
                RadIR=(Raw - bb4)/m4
                myticks=[0,50, 100,150,200]
                mylabels=["0","50","100","150","200"]
            if type == 'TMP':
                RadIR=(Raw - bb4)/m4
    ...
    # Add Colorbar
#   cbar = m.colorbar(cs, location='bottom', pad='0%', size='2%')
...
#   cbar.set_label(myunits)
    ...
    #f.colorbar(colormap=cmap, orientation='horizontal', shrink=0.625, aspect=20, fraction=0.2,pad=0.02)
    #f=plt.colorbar()
